...
{
	loadimage(&andimg, _T("and.png"));//使用库函数，读取组件的图像
...
void printer::print_widgets(unordered_map<int, widget*> &widgets)//绘制组件
{
	...
	for (it = widgets.begin(); it != widgets.end(); it++)//遍历所有组件
	{
		...
		case 1:
			if (it->second->isFocused())//如果组件处于被选中状态
				print_andf(it->second->get_x(), it->second->get_y());//绘制被选中状态的图像
			else
				print_and(it->second->get_x(), it->second->get_y());//否则，绘制未被选中状态的图像
			break;
		case 2:
...
void printer::print_lines(unordered_map<int, widget*>& widgets)//绘制连线
{
	unordered_map<int, widget*>::iterator it;
	for (it = widgets.begin(); it != widgets.end(); it++)//遍历每个组件
	{
		for (int i = 0; i < it->second->getN(); i++)//对于该组件的每个输入管脚
		{
			if(it->second->getPre(i)!=-1)//如果有连接东西
				print_line(widgets[it->second->getPre(i)]->getOutputX(), widgets[it->second->getPre(i)]->getOutputY(), it->second->getInputX(i), it->second->getInputY(i));//绘制连线
		}
	}
}
...
void printer::print_outputvalue(unordered_map<int, widget*>& widgets)//绘制每个组件的输出值
{
	unordered_map<int, widget*>::iterator it;
	for (it = widgets.begin(); it != widgets.end(); it++)//遍历每个组件
	{
		...
		if (it->second->getOutputValue(widgets) != -1)//如果该组件的有输出值
		{
			...
			outtextxy(it->second->getOutputX(), it->second->getOutputY() - 20, v);//调用库的函数，绘制输出值
		}
		else
			outtextxy(it->second->getOutputX(), it->second->getOutputY() - 20, '-');//没有输出值，调用库的函数绘制一个“-”号
	}
}